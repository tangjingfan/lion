...
**cross-floor 是唯一的硬删除**，而且现在按 sub-path 粒度做：每条
sub-trajectory 若 `max(y) − max(y) > 0.5 m` 就从 `survivor.sub_paths`
里删掉，但只要这个 episode 还有任何一条 sub-path 留在同一层，episode
本身就保留。过了 stage 01，留下来的每条 sub-path 都会在
`survivor.sub_paths` 里坚持到最后；后面 stage 把失败**标签化**到
...
按 sub-path 粒度丢掉竖直跨度 > `--threshold_m`（默认 0.5 m）的
sub-trajectory。只要一个 episode 还有任何一条 sub-path 在同层，
episode 本身就保留；只有当一个 episode 的**全部** sub-path 都跨楼层
时才整集丢弃。drop yaml 里记录每个被丢掉的 `(ep, sub_idx)` 以及它的
Δy。
