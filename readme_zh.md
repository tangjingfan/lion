...
5. **整合 dataset** — 把每条存活 sub-trajectory 的 text / 几何 / target / rescue 全部拼成 `dataset.json`。
...
把 rescue 结果合回 target_instances.json（第 4 章，脚本 10）
        │
        ▼
整合每条 sub-trajectory → dataset.json（第 5 章，脚本 11）
```
...
- `results/{run}/dataset.json` —— 顶层是 JSON list，每条对应一个存活的 `(scan, instruction_id, sub_idx)`。每条记录是 rewrite / partition / target_instances 三个 JSON 的字段并集，外加从 Landmark-RxR 数据集直接拿到的 instruction 原文。
