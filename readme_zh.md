...
`current.yaml` 是下游所有工具通过 `--exp` 读的入口。
...
注意：`filters/current.yaml` / `03_partition.yaml` 这类 survivor 文件要传给 `--exp`，不要传给 `--config`。`--config` 只用于 `configs/rollout/rollout_landmark_rxr.yaml` 这种 rollout 配置。
