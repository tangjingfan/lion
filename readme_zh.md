...
4. **Filter pipeline 10 + rescue** — 基于最终 target 做语义粒度过滤，并可选用 YOLO-World（VLM 兜底）救回部分粗粒度实例。
...
target instance selection（第 3 章，脚本 05-08）
        │
        ▼
可选：VLM semantic rescue（第 4 章，脚本 09）
        │
        ▼
最终 filter：semantic_granularity（第 4 章，脚本 10）
```
...
## 4. Filter pipeline（rescue + 步骤 10）
...
- `survivor.yaml` —— 覆盖为通过语义粒度检查和 rescue 检查的 survivor set。
